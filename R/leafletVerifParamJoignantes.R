leafletVerifParamJoignantes <-
function(data,fondMaille,typeMaille,fondSuppl,idDataDepart,idDataArrivee,varFlux,largeurFlecheMax,filtreVol,filtreDist,filtreMajeurs,decalageAllerRetour,decalageCentroid,colFleche,colBorder,dom,map_proxy){
  ...
  if(any(class(colFleche)!="character")) msg_error14 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colBorder)!="character")) msg_error15 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(dom)!="character")) msg_error16 <- "La valeur doit etre de type caractere ('0', '971', '972', '973', '974' ou '976') / "
  
  if(length(names(data))<3) msg_error17 <- "Le tableau des donnees n'est pas conforme. Il doit contenir au minimum une variable de depart, une variable d'arrivee et la variable a representer / "
  if(length(names(fondMaille))<3) msg_error18 <- "Le fond de maille n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  ...
  if(!typeMaille %in% c("REG", "DEP", "ZE", "AU", "BV", "UU", "EPCI", "COM")) msg_error20 <- "La variable typeMaille doit etre 'REG', 'DEP', 'ZE', 'AU', 'BV', 'UU', 'EPCI' ou 'COM' / "
