...
function(data,fondMaille,fondMailleElargi,fondSuppl,idData,varVolume,varRatio,rayonRond,rapportRond,methode,nbClasses,bornes,stylePalette,opacityElargi,colBorder,precision,dom,fondChx,map_proxy){
  ...
  if(any(class(colBorder)!="character")) msg_error16 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(precision)!="numeric")) msg_error17 <- "La variable doit etre de type numerique / "
  if(!is.null(fondChx)) if(any(!any(class(fondChx) %in% "sf"),!any(class(fondChx) %in% "data.frame"))) msg_error18 <- "Le fond des chx doit etre un objet sf / "
  
  if(length(names(data))<2) msg_error19 <- "Le tableau des donnees n'est pas conforme. Il doit contenir au minimum une variable identifiant et la variable a representer / "
  ...
  if(!is.null(fondMailleElargi)) if(length(names(fondMailleElargi))<3) msg_error21 <- "Le fond de maille elargie n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  if(!is.null(fondSuppl)) if(length(names(fondSuppl))<3) msg_error22 <- "Le fond supplementaire n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  
  if(!any(names(data) %in% idData))  msg_error23 <- "La variable identifiant les donnees n'existe pas dans la table des donnees / "
  if(!any(names(data) %in% varVolume))  msg_error24 <- "La variable a representer n'existe pas dans la table des donnees / "
  if(!any(names(data) %in% varRatio))  msg_error25 <- "La variable a representer n'existe pas dans la table des donnees / "
  if(!methode %in% c("kmeans","fisher","jenks","quantile")) msg_error26 <- "Le nom de la methode doit etre 'kmeans', 'fisher', 'jenks' ou 'quantile' / "
  if(!dom %in% c("0","971","972","973","974","976")) msg_error27 <- "La variable dom doit etre '0', '971', '972', '973', '974' ou '976' / "
  
  if(!is.null(map_proxy)) if(any(class(map_proxy)!="character")) if(!any(class(map_proxy) %in% "leaflet_proxy")) msg_error28 <- "Le parametre map_proxy doit etre un objet leaflet_proxy ou une chaine de caractere (voir aide) / "
         ...
         !is.null(msg_error25),!is.null(msg_error26),!is.null(msg_error27),!is.null(msg_error28)))
  {
    stop(simpleError(paste0(msg_error1,msg_error2,msg_error3,msg_error4,msg_error5,msg_error6,msg_error7,msg_error8,
                            ...
                            msg_error25,msg_error26,msg_error27,msg_error28)))
  }
}
