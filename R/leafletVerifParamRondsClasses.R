leafletVerifParamRondsClasses <-
function(data,fondMaille,fondMailleElargi,fondSuppl,idData,varVolume,varRatio,rayonRond,rapportRond,methode,nbClasses,bornes,stylePalette,opacityElargi,colBorderPos,colBorderNeg,epaisseurBorder,precision,emprise,fondEtranger,fondChx,map_proxy){
  ...
  if(any(class(colBorderPos)!="character")) msg_error16 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colBorderNeg)!="character")) msg_error17 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(epaisseurBorder)!="numeric")) msg_error18 <- "L'epaisseur de la bordure doit etre de type numerique / "
  if(any(class(precision)!="numeric")) msg_error19 <- "La variable doit etre de type numerique / "
  if(!is.null(fondChx)) if(any(!any(class(fondChx) %in% "sf"),!any(class(fondChx) %in% "data.frame"))) msg_error20 <- "Le fond des chx doit etre un objet sf / "
  ...
  if(!is.null(fondMailleElargi)) if(length(names(fondMailleElargi))<3) msg_error24 <- "Le fond de maille elargie n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  if(!is.null(fondSuppl)) if(length(names(fondSuppl))<3) msg_error25 <- "Le fond supplementaire n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  if(!is.null(fondEtranger)) if(length(names(fondEtranger))<3) msg_error26 <- "Le fond etranger n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  
  if(!any(names(data) %in% idData))  msg_error27 <- "La variable identifiant les donnees n'existe pas dans la table des donnees / "
  if(!any(names(data) %in% varVolume))  msg_error28 <- "La variable a representer n'existe pas dans la table des donnees / "
  if(!any(names(data) %in% varRatio))  msg_error29 <- "La variable a representer n'existe pas dans la table des donnees / "
  if(!methode %in% c("kmeans","fisher","jenks","quantile")) msg_error30 <- "Le nom de la methode doit etre 'kmeans', 'fisher', 'jenks' ou 'quantile' / "
  if(!emprise %in% c("FRM","971","972","973","974","976","999")) msg_error31 <- "La variable emprise doit etre 'FRM', '971', '972', '973', '974', '976' ou '999' / "
  
  if(!is.null(map_proxy)) if(any(class(map_proxy)!="character")) if(!any(class(map_proxy) %in% "leaflet_proxy")) msg_error32 <- "Le parametre map_proxy doit etre un objet leaflet_proxy ou une chaine de caractere (voir aide) / "
