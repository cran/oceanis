leaflet_joignantes <-
function(data,fondMaille,typeMaille,fondSuppl=NULL,idDataDepart,idDataArrivee,varFlux,largeurFlecheMax=NULL,filtreVol=0,filtreDist=100,filtreMajeurs=10,decalageAllerRetour=0,decalageCentroid=0,colFleche="#286AC7",colBorder="#303030",emprise="FRM",fondEtranger=NULL,zoomMaille=NULL,map_proxy=NULL)
  {
    options("stringsAsFactors"=FALSE)

    # Verification des parametres
    leafletVerifParamJoignantes(data,fondMaille,typeMaille,fondSuppl,idDataDepart,idDataArrivee,varFlux,largeurFlecheMax,filtreVol,filtreDist,filtreMajeurs,decalageAllerRetour,decalageCentroid,colFleche,colBorder,emprise,fondEtranger,map_proxy)
    ...
    if(!is.null(map_proxy))
    {
      ...
      {
        clearGroup(map_proxy, group = "carte_joignantes")
      }
    }
    ...
    if(is.null(map_proxy) | (!is.null(map_proxy) & class(map_proxy)=="character"))
    {
      ...
      map <- leaflet(padding = 0,
        ...
        addMapPane(name = "fond_joignantes", zIndex = 406) %>%
    ...
    map <- addPolygons(map = map,
                       ...
                       layerId = list(analyse_WGS84=analyse_WGS84,donnees=donnees,code_epsg=code_epsg,emprise=emprise,nom_fond="fond_flux",var_flux=varFlux,max_var=max(abs(donnees[,varFlux])),largeur=largeurFlecheMax,distance=large_pl)
    )
