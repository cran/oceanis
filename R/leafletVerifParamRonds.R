leafletVerifParamRonds <-
function(data,fondMaille,fondMailleElargi,fondSuppl,idData,varVolume,rayonRond,rapportRond,dom,fondChx,colPos,colNeg,colBorder,opacityElargi,map_proxy){
  ...
  if(any(class(colBorder)!="character")) msg_error9 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colPos)!="character")) msg_error10 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(colNeg)!="character")) msg_error11 <- "Le style de couleur doit etre de type caractere / "
  if(any(class(opacityElargi)!="numeric")) msg_error12 <- "La variable doit etre de type numerique / "
  if(any(class(dom)!="character")) msg_error13 <- "La valeur doit etre de type caractere ('0', '971', '972', '973', '974' ou '976') / "
  if(!is.null(fondChx)) if(any(!any(class(fondChx) %in% "sf"),!any(class(fondChx) %in% "data.frame"))) msg_error14 <- "Le fond des chx doit etre un objet sf / "
  
  if(length(names(data))<2) msg_error15 <- "Le tableau des donnees n'est pas conforme. Il doit contenir au minimum une variable identifiant et la variable a representer / "
  ...
  if(!is.null(fondMailleElargi)) if(length(names(fondMailleElargi))<3) msg_error17 <- "Le fond de maille elargie n'est pas conforme. La table doit contenir au minimum une variable identifiant, une variable libelle et la geometry / "
  ...
  if(!any(names(data) %in% idData))  msg_error19 <- "La variable identifiant les donnees n'existe pas dans la table des donnees / "
  if(!any(names(data) %in% varVolume))  msg_error20 <- "La variable a representer n'existe pas dans la table des donnees / "
  if(!dom %in% c("0","971","972","973","974","976")) msg_error21 <- "La variable dom doit etre '0', '971', '972', '973', '974' ou '976' / "
  
  if (!is.null(map_proxy)) if (any(class(map_proxy)!="character")) if(!any(class(map_proxy) %in% "leaflet_proxy")) msg_error22 <- "Le parametre map_proxy doit etre un objet leaflet_proxy ou une chaine de caractere (voir aide) / "
  ...
  if(any(!is.null(msg_error1),!is.null(msg_error2),!is.null(msg_error3),!is.null(msg_error4),
         ...
         !is.null(msg_error21),!is.null(msg_error22)))
  {
