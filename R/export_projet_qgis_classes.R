...
function(liste_fonds,chemin_fonds,nom_projet,titre,titre2,sourc,titre_leg_classes,table_classe,variable_a_representer,annee)
  {
    ...
    for (i in 1:length(l))
    {
      ...
      }else
      {
        ...
        if (l[i] %in% c("fond_departement","fond_region","fond_pays","fond_etranger"))
        {
        ...
        stylebordure="solid"
        if (l[i] %in% c("fond_classes","fond_lignes","fond_departement","fond_pays","fond_etranger","fond_territoire"))
        {
