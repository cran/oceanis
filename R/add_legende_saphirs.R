...
function(map,titre=NULL,lng=NULL,lat=NULL,precision=0,zoom=8,map_leaflet=NULL)
  {
    ...
    }else
    {
      ...
      {
        large <- c(large,as.numeric(max(st_distance(st_sfc(st_point(c(coord_fleche_max[[i]][2,1],coord_fleche_max[[i]][2,2])),st_point(c(coord_fleche_max[[i]][6,1],coord_fleche_max[[i]][6,2])), crs = 4326)))))
      }
