...
function(map,titre=NULL,lng=NULL,lat=NULL,precision=0,zoom=8,map_leaflet=NULL)
  {
    ...
    if(any(class(map) %in% "leaflet"))
    {
      ...
      for(i in 1:length(map$x$calls))
      {
        ...
        {
          if(any(map$x$calls[[i]]$args[3][[1]] %in% c("carte_saphirs"))) idx_fleche <- i
        }

        if(map$x$calls[[i]]$method %in% "addRectangles")
        {
          if(map$x$calls[[i]]$args[[6]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
        }
        if(!is.null(idx_legende))
        {
          if(map$x$calls[[i]]$method %in% "addPolygons")
          {
            if(map$x$calls[[i]]$args[3][[1]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
          }
          if(!is.null(idx_legende)) # la legende existe
          {
            if(map$x$calls[[i]]$method %in% "addMarkers")
            {
              if(map$x$calls[[i]]$args[5][[1]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
            }
          }
        }
      }
      ...
      colEntree <- map$x$calls[[idx_fleche]]$args[[2]]$colEntree
      colSortie <- map$x$calls[[idx_fleche]]$args[[2]]$colSortie

      if(is.null(colEntree) & !is.null(colSortie))
      {
        colRemp <- colSortie
      }else if(!is.null(colEntree) & is.null(colSortie))
      {
        colRemp <- colEntree
      }else
      {
      ...
      coord_fleche_max <- list()
      for(i in 1:length(map$x$calls[[idx_fleche]]$args[[1]]))
      {
        coord_fleche_max[[i]] <- data.frame(lng=map$x$calls[[idx_fleche]]$args[[1]][[i]][[1]][[1]]$lng,lat=map$x$calls[[idx_fleche]]$args[[1]][[i]][[1]][[1]]$lat)
      }
    ...
    }else if(any(class(map) %in% "leaflet_proxy"))
    {
      ...
      for(i in 1:length(map_leaflet$x$calls))
      {
        ...
        {
          if(any(map_leaflet$x$calls[[i]]$args[3][[1]] %in% c("carte_saphirs"))) idx_fleche <- i
        }

        if(map_leaflet$x$calls[[i]]$method %in% "addRectangles")
        {
          if(map_leaflet$x$calls[[i]]$args[[6]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
        }
        if(!is.null(idx_legende))
        {
          if(map_leaflet$x$calls[[i]]$method %in% "addPolygons")
          {
            if(map_leaflet$x$calls[[i]]$args[3][[1]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
          }
          if(!is.null(idx_legende)) # la legende existe
          {
            if(map_leaflet$x$calls[[i]]$method %in% "addMarkers")
            {
              if(map_leaflet$x$calls[[i]]$args[5][[1]]=="legende_saphirs") idx_legende <- c(idx_legende,i)
            }
          }
        }
      }
      ...
      colEntree <- map_leaflet$x$calls[[idx_fleche]]$args[[2]]$colEntree
      colSortie <- map_leaflet$x$calls[[idx_fleche]]$args[[2]]$colSortie
      ...
      coord_fleche_max <- list()
      for(i in 1:length(map_leaflet$x$calls[[idx_fleche]]$args[[1]]))
      {
        coord_fleche_max[[i]] <- data.frame(lng=map_leaflet$x$calls[[idx_fleche]]$args[[1]][[i]][[1]][[1]]$lng,lat=map_leaflet$x$calls[[idx_fleche]]$args[[1]][[i]][[1]][[1]]$lat)
      }

      clearGroup(map, group = "legende_saphirs")
    }
    ...
    {
      large <- NULL
      for(i in 1:length(coord_fleche_max))
      {
        large <- c(large,max(st_distance(st_sfc(st_point(c(coord_fleche_max[[i]][2,1],coord_fleche_max[[i]][2,2])),st_point(c(coord_fleche_max[[i]][6,1],coord_fleche_max[[i]][6,2]))))))
      }
      large <- max(large)
      long <- coeff*2
      ...
      map <- addRectangles(map = map,
                           ...
                           lng2 = st_bbox(flux_legWGS84)[3]+coeff*3, lat2 = st_bbox(flux_legWGS84)[4]+coeff,
                           stroke = FALSE,
                           options = pathOptions(pane = "fond_legende", clickable = F),
                           fill = T,
                           fillColor = "white",
                           fillOpacity = 0.8,
                           group="legende_saphirs"
      )
      ...
      #leaflet titre 1
      map <- addLabelOnlyMarkers(map = map,
                                 lng = st_bbox(flux_legWGS84)[1], lat = st_bbox(flux_legWGS84)[4]+coeff/2,
                                 label = titre,
                                 labelOptions = labelOptions(noHide = T, textOnly = TRUE, direction = "right",
                                                             style = list(
                                                               "color" = "black",
