    ...
    server <- function(input, output, session) {
      ...
      observe({
          ...
          output$slider_bornes_ac_rp <- renderUI({
            lapply(1:(as.numeric(input$nb_classes_ac_rp_id)-1)+1, function(i) {
              sliderInput(inputId = paste0("slider_bornes_", i,"_ac_rp_id"), label = NULL,
                          value = rev(react_bornes_ac_rp()[[1]])[i], min = round(min(react_bornes_ac_rp()[[1]]),3), max = round(max(react_bornes_ac_rp()[[1]]),3), step = 0.001) #min = rev(react_bornes_ac_rp()[[1]])[i-1], max = rev(react_bornes_ac_rp()[[1]])[i+1]
            })
          })

          output$valid_slider_bornes_ac_rp <- renderUI({
            actionButton("valid_slider_bornes_ac_rp_id",label=label_bouton_ac_rp(), icon=icon("sync"), style="color: #fff; background-color: #337ab7; border-color: #2e6da4")
          })
        },ignoreInit = TRUE)
        ...
        output$valid_bornes_ac_rp <- renderUI({
          actionButton("valid_bornes_ac_rp_id",label="Rafraichir la carte", icon=icon("sync"), style="color: #fff; background-color: #337ab7; border-color: #2e6da4")
        })
      ...
      calcul_rond_ac_rp <- reactive({
        ...
        if(!is.null(input$taille_rond_ac_rp_id))
        {
          ...
          {
            showModal(modalDialog(HTML(paste0("Le rayon du rond le plus grand est trop \u00e9lev\u00e9 et ne permet pas de respecter la r\u00e8gle s\u00e9miologique des 1/7\u00e8me. Le rayon max conseill\u00e9 est ",round(calcul_max_rayon_metres_ac_rp()[[1]],2)," m\u00e8tres.")), size="l", footer=NULL, easyClose = TRUE, style = "color: #fff; background-color: #DF691A; border-color: #2e6da4")) #337ab7
          }
        ...
        output[[paste0("mymap_save_",insert_save$a,"_ac_rp")]] <- renderLeaflet({
          ...
          if(!is.null(fondSuppl))
          {
            if(isolate(input$ajout_territoire_ac_rp_id))
            {
              m_save <- addPolygons(map = m_save, data = isolate(fond_territoire_ac_rp()),
                                    ...
                                    weight = 0.5,
                                    options = pathOptions(pane = "fond_territoire", clickable = T),
                                    popup = paste0("<b> <font color=#2B3E50>",as.data.frame(isolate(fond_territoire_ac_rp()))[,"LIBELLE"], "</font> </b>"),
                                    fill = T, fillColor = "white", fillOpacity = 0.001
              )
            }
          }
          ...
          for(fond in isolate(liste_fonds$a))
          {
            if(fond=="analyse")
            {
              m_save <- addCircles(map = m_save,
                                   ...
                                   options = pathOptions(pane = paste0("fond_trio",i), clickable = T),
                                   popup = paste0("<b> <font color=#2B3E50>",varVolume," : </font></b>",isolate(analyse_ac_rp())[[1]]$donnees$TXT1),
                                   fill = F
              )
            }
            ...
            if(fond=="contour")
            {
              m_save <- addPolygons(map = m_save, data = isolate(fond_contour_maille_ac_rp())[[1]], opacity = 0.3, #contour_WGS84
                                    stroke = TRUE, color = "black", weight = 3,
                                    options = pathOptions(pane = paste0("fond_trio",i), clickable = T),
                                    popup = paste0("<b> <font color=#2B3E50>",as.data.frame(isolate(fond_contour_maille_ac_rp())[[1]])[,"LIBELLE"], "</font> </b>"),
                                    fill = T, fillColor = "white", fillOpacity = 0.3
              )
            }
          ...
          if(isolate(elargi_ac_rp()))
          {
            ...
            m_save <- addCircles(map = m_save,
                                 ...
                                 options = pathOptions(pane = "fond_trio3", clickable = T),
                                 popup = paste0("<b> <font color=#2B3E50>",varVolume," : </font></b>",isolate(analyse_ac_rp())[[1]]$donnees_elargi$TXT1),
                                 fill = F
            )
          }
