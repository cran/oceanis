...
function(data,fondMaille,fondMailleElargi,fondSuppl,idData,varRatio,methode,nbClasses,bornes,stylePalette,opacityElargi,colBorder,precision,emprise,fondEtranger,map_proxy){
  ...
  if(!any(names(data) %in% idData))  msg_error21 <- "La variable identifiant les donnees n'existe pas dans la table des donnees / "
